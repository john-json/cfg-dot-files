...
for i = 1, 10 do
	...
	space:subscribe("front_app_switched", function(env)
		...
		local targetColor = selected and getSpaceColor(i) or colors.bar.accent_transparent
		...
		sbar.animate("elastic", 10, function()
			space:set({
				background = {
					...
					drawing = true,
					color = selected and getSpaceColor(i) or colors.bar.accent_transparent,
					height = selected and 18 or 16,
					corner_radius = 25,
				},
				label = {
					padding_left = selected and 20 or 7,
					padding_right = selected and 20 or 7,
	...
	space:subscribe("mouse.entered", function(env)
		...
		sbar.delay(0.2, function()
			sbar.animate("elastic", 10, function()
				space:set({
					label = {
						padding_left = selected and 22 or 12,
						padding_right = selected and 22 or 12,
						string = selected and getSpaceIcon(i, false) or getSpaceIcon(i, true),
						color = selected and getSpaceColor(i) or colors.bar.accent_transparent,
					},
					background = {
			...
			sbar.animate("elastic", 10, function()
				space:set({
					...
					label = {
						...
						padding_right = selected and 20 or 7,
						string = selected and getSpaceIcon(i, false) or getSpaceIcon(i, true),
						color = selected and getSpaceColor(i) or colors.bar.acccent
					},
				})
			end)
		end)
